...
if [ "$(id -u)" = "0" ]; then
    ...
    echo -e "\nUse $(green "bash PERSONAcommander.sh") as a REGULAR user instead"
    echo -e "Execute ONCE $(green "chmod +x PERSONAcommander.sh") followed by $(green "ENTER")"
    echo -e "and start it only by $(green "./PERSONAcommander.sh") as regular user after\n"
    exit 1
fi
...
if [ $(systemd-detect-virt -c) != "none" ]; then
    ...
    echo "$(ired "                                                                                 ")"
    echo "$(ired "     Running Persona Node on a Container based virtual system is not recommended!    ")"
...
cat << "EOF"
 ...
 ___________ ___________  _____  _____ _   _   ___  
...
function proc_vars {
        node=`pgrep -a "node" | grep personatestnet | awk '{print $1}'`
        if [ "$node" == "" ] ; then
                node=0
        fi
        ...
        personadir=`locate -b -l 1 "\personatestnet"`
...
#   echo -e "$(green "Public Key:")\n$(yellow "$PUBKEY")\n"
    ...
    echo -e "$(green "      Persona Balance      : ")$(yellow "$BALANCE")"
    echo
    echo -e "\n$(yellow "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")"
        if [ -e $personadir/app.js ]; then
                echo -e "\n$(green "       ? Persona Node installation found!")\n"
                if [ "$node" != "" ] && [ "$node" != "0" ]; then
                        echo -e "$(green "      Persona Node process is running with:")"
                        echo -e "$(green "   System PID: $node, Forever PID $forever_process")"
                        echo -e "$(green "   and Work Directory: $personadir")\n"
                else
                        echo -e "\n$(red "       ? No Persona Node process is running")\n"
                fi
        else
                echo -e "\n$(red "       ? No Persona Node installation is found")\n"
        fi
    echo -e "\n$(yellow "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")"
...
function stats {
    ...
    if [ "$node" != "" ] && [ "$node" != "0" ]; then
        echo -e "$(green "       Instance of Persona Node found with:")"
        echo -e "$(green "       System PID: $node, Forever PID $forever_process")"
...
function ntpd {
        ...
        if [ $(systemd-detect-virt) == "lxc" ] || [ $(systemd-detect-virt) == "openvz" ]; then
            ...
            sleep 2
                                if ! sudo pgrep -x "ntpd" > /dev/null; then
                                        echo -e "NTP failed to start! It should be installed and running for Persona.\n Check /etc/ntp.conf for any issues and correct them first! \n Exiting."
                                        exit 1
                                fi
...
# Logrotate for Persona Node logs
function log_rotate {
    if [[ "$(uname)" == "Linux" ]]; then

        if [ ! -f /etc/logrotate.d/persona-logrotate ]; then
            echo -e " Setting up Logrotate for Persona Node log files."
            sudo bash -c "cat << 'EOF' >> /etc/logrotate.d/persona-logrotate
$personadir/logs/persona.log {
        size=50M
        copytruncate
...
function git_upd_check {
    ...
    if [ -d "$personadir" ]; then
        ...
        if [ "$LOCAL" == "$REMOTE" ]; then
            echo -e "         $(igreen "    Persona Node is Up-to-date    \n")"
            UP_TO_DATE=1
...
function purge_pgdb {
    ...
    echo -e "    $(ired "   running Persona Node processes and will  ")"
...
if [ "$(ls -A $SNAPDIR)" ]; then
    if [[ $(expr `date +%s` - `stat -c %Y $SNAPDIR/current`) -gt 900 ]]; then
        echo -e "$(yellow " Existing Current snapshot is older than 15 minutes")"
            read -e -r -p "$(yellow "\n Download from persona.im? (Y) or use Local (N) ")" -i "Y" YN
            if [[ "$YN" =~ [Yy]$ ]]; then
                echo -e "$(yellow "\n     Downloading latest snapshot from persona.im\n")"
                rm $SNAPDIR/current
                wget -nv https://snapshots.persona.im/current -O $SNAPDIR/current
                echo -e "$(yellow "\n              Download finished\n")"
            fi
    fi
        ...
        ## Numeric checks
            ...
            pg_restore -O -j 8 -d persona_testnet $SNAPDIR/${snapshots[$(($REPLY-1))]} 2>/dev/null
        ...
        if [[ "$YN" =~ [Yy]$ ]]; then
        echo -e "$(yellow "\n     Downloading current snapshot from persona.im\n")"
                wget -nv https://snapshots.persona.im/current -O $SNAPDIR/current
        echo -e "$(yellow "\n              Download finished\n")"
        fi
        ...
        if [[ $? -eq 0 ]]; then
                ...
                echo -e "$(yellow "\n   Restoring $SNAPDIR/current ... ")"
                                pg_restore -O -j 8 -d persona_testnet $SNAPDIR/current 2>/dev/null
...
function inst_persona {
#   proc_vars
    cd $HOME
        mkdir personatestnet
        git clone https://github.com/PersonaIam/personatestnet 2>/dev/null
        cd personatestnet
...
function create_db {
        ...
        sleep 1
#       sudo -u postgres dropdb --if-exists persona_testnet
#       sleep 1
...
function db_exists {
        ...
        if [[ ! $(sudo -u postgres psql persona_testnet -c '\q' 2>&1) ]]; then
...
function update_persona {
    if [ "$UP_TO_DATE" -ne 1 ]; then
            cd $personadir
#            forever stop app.js
        TMP_PASS=$(jq -r '.forging.secret | @csv' config.testnet.json)
        ...
        if [ ! -e config.testnet.json ]; then
            ...
            jq -r '.forging.secret = ['"$TMP_PASS"']' config.testnet.json > config.testnet.tmp && mv config.testnet.tmp config.testnet.json
        fi
...
#           forever start app.js --genesis genesisBlock.testnet.json --config config.testnet.json
        ...
        echo "Persona Node is already up to date!"
        sleep 2
    fi
}
...
function secret {
    ...
    #Put check if personadir is empty, if it is stays only config.testnet.json
    echo -e "$(yellow " Enter (copy/paste) your private key (secret)")"
    echo -e "$(yellow "    (WITHOUT QUOTES!) followed by 'Enter'")"
    read -e -r -p ": " secret

    cd $personadir
    jq -r ".forging.secret = [\"$secret\"]" config.testnet.json > config.testnet.tmp && mv config.testnet.tmp config.testnet.json
}
...
one(){
    ...
    if [ -e $personadir/app.js ]; then
        clear
        asciiart
        echo -e "\n$(green "       ? Persona Node is already installed!")\n"
        if [ "$node" != "" ] && [ "$node" != "0" ]; then
                    echo -e "$(green "A working instance of Persona Node is found with:")"
                    echo -e "$(green "System PID: $node, Forever PID $forever_process")"
                    echo -e "$(green "and Work Directory: $personadir")\n"
                fi
        pause
        ...
        clear
        asciiart
        echo -e "$(yellow "           Installing Persona Node....")"
        create_db
        inst_persona
        clear
        asciiart
        echo -e "$(green "          ? Persona Node was installed")\n"
        sudo updatedb
        sleep 1
        proc_vars
        log_rotate
...
# Reinstall Persona Node
two(){
    clear
    asciiart
    echo -e "$(ired "!!! This option will erase your DB and Persona Node installation !!!")\n"
    ...
    if [ "$keys" == "Y" ]; then
        proc_vars
            if [ -e $personadir/app.js ]; then
                    clear
                    asciiart
                    echo -e "\n$(green " ? Persona Node installation found in $personadir")\n"
                    if [ "$node" != "" ] && [ "$node" != "0" ]; then
                            echo -e "$(green "A working instance of Persona Node is found with:")"
                            echo -e "$(green "System PID: $node, Forever PID $forever_process")"
                echo -e "$(yellow "           Stopping Persona Node ...")\n"
                cd $personadir
            ...
            if [ -e $parent/config.testnet.json ] ; then
                ...
                if [ "$keys" == "Y" ]; then
                    cp $personadir/config.testnet.json $parent
                    cd $parent
                fi
            else
                cp $personadir/config.testnet.json $parent
            ...
            echo -e "$(yellow "        Removing Persona Node directory...")\n"
            sleep 1
            rm -rf $personadir
            drop_db
            drop_user
...
#               echo "Break1"; pause
                if [ "$keys" == "Y" ]; then
                    cp $parent/config.testnet.json $personadir
                    echo -e "\n$(green " ? Config was restored in $personadir")\n"
                    read -e -r -p "$(yellow " Do you want to start Persona Node now? (Y/N): ")" -i "Y" keys
                    if [ "$keys" == "Y" ]; then
                        start
                    fi
                else
...
three(){
    ...
    if [ "$UP_TO_DATE" -ne 1 ]; then
            ...
            echo -e "         $(igreen " Persona Node is already Up-to-date \n")"
            sleep 2
    fi
...
four(){
        ...
        snap_menu
                echo -e "$(green "            Starting Persona Node...")"
        forever start app.js --genesis genesisBlock.testnet.json --config config.testnet.json >&- 2>&-
                echo -e "\n$(green "    ? Persona Node was successfully started")\n"
                pause
        else
                echo -e "\n$(red "       ? Persona Node process is not running")\n"
                echo -e "$(green "             Dropping Persona DB...")\n"
...
five(){
    ...
    if [ "$keys" == "Y" ]; then
            if [ "$node" != "" ] && [ "$node" != "0" ]; then
            echo -e "\n$(green "       Instance of Persona Node found with:")"
            echo -e "$(green "       System PID: $node, Forever PID $forever_process")"
            echo -e "$(green "       Directory: $personadir")\n"
            echo -e "$(green "                Restarting...")"
                    forever restart $forever_process >&- 2>&-
...
start(){
        ...
        if [ -e $personadir/app.js ]; then
                ...
                echo -e "\n$(green "       ? Persona Node installation found!")\n"
                if [ "$node" != "" ] && [ "$node" != "0" ]; then
                        echo -e "$(green " A working instance of Persona Node was found with:")"
                        echo -e "$(green "   System PID: $node, Forever PID $forever_process")"
                        echo -e "$(green "   and Work Directory: $personadir")\n"
        else
            echo -e "$(green "            Starting Persona Node...")\n"
            ...
            sleep 1
            proc_vars
            echo -e "\n$(green "       Persona Node started with:")"
            echo -e "$(green "   System PID: $node, Forever PID $forever_process")"
            echo -e "$(green "   and Work Directory: $personadir")\n"
                fi
    else
...
status(){
        ...
        if [ -e $personadir/app.js ]; then
                ...
                echo -e "\n$(green "       ? Persona Node installation found!")\n"
                if [ "$node" != "" ] && [ "$node" != "0" ]; then
                        echo -e "$(green "      Persona Node process is working with:")"
                        echo -e "$(green "   System PID: $node, Forever PID $forever_process")"
                        echo -e "$(green "   and Work Directory: $personadir")\n"
                else
                        echo -e "\n$(red "       ? No Persona Node process is running")\n"
...
killit(){
        ...
        if [ -e $personadir/app.js ]; then
                ...
                if [ "$node" != "" ] && [ "$node" != "0" ]; then
                        ...
                        echo -e "$(green "   System PID: $node, Forever PID $forever_process")"
                        echo -e "$(green "   and Work Directory: $personadir")\n"
            echo -e "$(green "            Stopping Persona Node...")\n"
            cd $personadir
            forever stop $forever_process >&- 2>&-
            cd $parent
            echo -e "$(green "    ? Persona Node was successfully stopped")\n"
                else
            echo -e "\n$(red "       ? No Persona Node process is running")\n"
                fi
        else
...
show_menus() {
    ...
    echo "         ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
    echo
    echo "              1. Install Persona"
    echo "              2. Reinstall Persona"
    echo "              3. Update Persona"
    ...
    echo "         ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
    echo
    echo "              A. Persona Start"
    echo "              R. Restart Persona"
    echo "              K. Kill Persona"
    echo "              S. Node Status"
        echo "              L. Node Log"
    echo "              0. Exit"
...
sub_menu() {
    ...
    echo "         ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
    echo
    echo "           1. Install Persona Cli"
    echo "           2. Install Persona Explorer"
...
if [ -e ./.firstrun ] && [ "$fdate" =  "$sdate" ]; then
    ...
    if [ ! -e ./.firstrun ] ; then
        ...
        echo -e "\n$(ired "    !!!  PLEASE REBOOT YOUR SYSTEM NOW  !!!    ") "
          ...
          echo -e "$(ired "    !!!  CHOOSE '1' TO INSTALL Persona Node !!!    ") "
        exit
    fi
fi
